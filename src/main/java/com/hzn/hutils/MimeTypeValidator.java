...
public class MimeTypeValidator {
	...
	public static boolean validate (MultipartFile multipartFile, MimeType... mimeTypes) throws IOException {
		return validate (MultipartFileConverter.toFileMap (multipartFile),
		                 Arrays.stream (mimeTypes).map (MimeType::getType).toList ().toArray (new String[mimeTypes.length]));
	}

	public static boolean validate (MultipartFile multipartFile, String... mimeTypes) throws IOException {
	...
	public static boolean validate (File file, MimeType... mimeTypes) throws IOException {
		return validate (Map.of (file, false), Arrays.stream (mimeTypes).map (MimeType::getType).toList ().toArray (new String[mimeTypes.length]));
	}
	...
	public static boolean validate (Map<File, Boolean> fileMap, MimeType... mimeTypes) throws IOException {
		return validate (fileMap, Arrays.stream (mimeTypes).map (MimeType::getType).toList ().toArray (new String[mimeTypes.length]));
	}

	public static boolean validate (Map<File, Boolean> fileMap, String... mimeTypes) throws IOException {
	...
	private static Optional<String> detectMimeType (File file) throws IOException {
		try (FileInputStream fis = new FileInputStream (file); FileChannel fc = fis.getChannel ()) {
			return Arrays.stream (MimeType.values ()).filter (mt -> mt.getMagicNumber () != null).filter (mt -> {
				try {
	...
	private static Optional<String> detectExtension (File file) {
		return Arrays.stream (MimeType.values ()).flatMap (mt -> Arrays.stream (mt.getExtensions ())).filter (e -> {
